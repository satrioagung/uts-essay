...
- `/login` — pilih Admin atau Siswa, lalu isi sembarang kredensial.
...
`lib/supabase/client.ts` dan `lib/supabase/server.ts` sudah siap untuk Supabase SSR. Custom session memakai httpOnly cookie melalui `app/api/auth/login/route.ts`; validasi impor terpisah di `lib/import-validation.ts`. Hubungkan query Supabase dan policies sesuai deployment sekolah sebelum produksi.
